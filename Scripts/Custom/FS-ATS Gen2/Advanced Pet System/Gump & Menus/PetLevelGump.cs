...
namespace Server.Gumps
{
	public class PetLevelGump : Gump
	{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( info.ButtonID == 2 )
			{
				if ( bc.StamMax - (pm.Paragon_PetStats*10) >= FSATS.NormalSTAM )
				{
					from.SendMessage( "This cannot gain any farther." );
			...
			if ( info.ButtonID == 3 )
			{
				if ( bc.ManaMax  - (pm.Paragon_PetStats*10) >= FSATS.NormalMANA )
				{
					from.SendMessage( "This cannot gain any farther." );
			...
			if ( info.ButtonID == 9 )
			{
				if ( (bc.DamageMin - pm.Paragon_1PetMinMaxDamage) >= FSATS.NormalMinDam )
				{
					from.SendMessage( "This cannot gain any farther." );
				...
				{
					bc.AbilityPoints -= 1;
					bc.DamageMin += (1 - pm.Paragon_1PetMinMaxDamage) ;
			...
			if ( info.ButtonID == 10 )
			{
				if ( (bc.DamageMax - pm.Paragon_1PetMinMaxDamage) >= FSATS.NormalMaxDam )
				{
					from.SendMessage( "This cannot gain any farther." );
				...
				{
					bc.AbilityPoints -= 1;
					bc.DamageMax += (1 - pm.Paragon_1PetMinMaxDamage);
