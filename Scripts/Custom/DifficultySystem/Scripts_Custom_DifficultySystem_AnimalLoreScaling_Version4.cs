...
namespace Server
{
        ...
        private class AnimalLoreScalingTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is BaseCreature bc)
                {
                    ...
                    int scaledCurrentHits = bc.GetPerceivedCurrentHits(from, healthMultiplier);
                    int scaledMaxHits = (int)(bc.HitsMax * healthMultiplier);

                    from.SendGump(new ScaledAnimalLoreGump(bc, scaledCurrentHits, scaledMaxHits));
                }
                else
    ...
    {
        public ScaledAnimalLoreGump(BaseCreature bc, int scaledCurrentHits, int scaledMaxHits) : base(250, 50)
        {
            AddPage(0);
