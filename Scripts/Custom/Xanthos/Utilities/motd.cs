/*
Modified by Alari - 1:52 AM 4/21/2007

Changed to support 9999 archived messages.
Changed to pad filenames with zeroes so that they load in the correct order.
Changed position of title to support longer titles.

1:04 AM 4/30/2007
Changed back to 9 archived messages, too many and it won't show up properly. =(  
...
namespace Xanthos.Utilities
{
	public class NewsGump : Gump
	{
		...
		private static void ArchiveMotd( PlayerMobile mobile ) // Not thread safe, call only from a locked block
		{
			...
			{
				if ( File.Exists( source = kPathArchive + String.Format( "{0}", i ) + kFileSuffix ) )
					File.Move( source, kPathArchive + String.Format( "{0}", j ) + kFileSuffix );
			}
		...
		private void AddNewPage( string bodyText )
		{
			...
			AddLabel( 90, 15, kDataColor, kGreeting );  // was 170...
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				...
				case 103:	// Account Gump
					SafelySendGumpTo( from, false );  // I don't think the acct gump should close the MOTD gump, there's no path back.. bad UI.
		...
		internal class AccountGump : Gump
		{
			public AccountGump( Mobile from, NetState state, bool showOnlyCredits ) : base( 30, 20 )
			{
				...
				if ( !showOnlyCredits )
				{
					...
					AddLabel( 205, 180, NewsGump.kDataColor, ( (state.Flags & ClientFlags.Tokuno) != 0 ) ? "Samurai Empire" : (( state.Flags & ClientFlags.Malas ) != 0) ? "Age of Shadows" : (( state.Flags & ClientFlags.Ilshenar ) != 0) ? "Blackthorn's Revenge" : (( state.Flags & ClientFlags.UOTD ) != 0 ) ? "Third Dawn" : "Renaissance/T2A" );
