...
namespace Arya.Chess
{
	...
	public abstract class BaseChessPiece
	{
		...
		/// </summary>
		protected Chssbrd m_Chessboard;
		/// <summary>
		...
		public virtual Direction Facing
		{
			get
			{
				if ( m_Chessboard.Orientation == BoardOrientation.NorthSouth )
				{
					if ( m_Color == ChessColor.Black )
		...
		public virtual int Hue
		{
			...
			{
				if ( m_Color == ChessColor.Black )
					return m_Chessboard.BlackHue;
				else
					return m_Chessboard.WhiteHue;
			}
		}
		...
		public virtual int MinorHue
		{
			...
			{
				if ( m_Color == ChessColor.Black )
					return m_Chessboard.BlackMinorHue;
				else
					return m_Chessboard.WhiteMinorHue;
			}
		}
		...
		public BaseChessPiece( Chssbrd board, ChessColor color, Point2D position )
		{
			m_Chessboard = board;
			m_Color = color;
			m_Position = position;
		...
		public virtual void MoveTo( Move move )
		{
			...
			if ( move.Capture )
			{
				m_Chessboard.PlaySound( m_Piece, m_CaptureSound );

				// It's a capture, do an effect
				m_Piece.MovingParticles( move.CapturedPiece.m_Piece, m_Chessboard.AttackEffect, 5, 0, false, true, Hue, 2, 0, 1, 4006, EffectLayer.Waist, 0 );
			}
			else
			{
				m_Chessboard.PlaySound( m_Piece, m_MoveSound );
			}
		...
		public virtual void OnMoveOver()
		{
			...
			if ( m_Move.Capture )
			{
				m_Piece.FixedParticles( m_Chessboard.CaptureEffect, 1, 15, 5012, SecondaryHue, 2, EffectLayer.Waist );
		...
		public virtual void Die( bool sound )
		{
			...
			{
				if ( m_Chessboard.BoltOnDeath )
					m_Piece.BoltEffect( SecondaryHue );

				m_Chessboard.PlaySound( m_Piece, m_DeathSound );
			}
