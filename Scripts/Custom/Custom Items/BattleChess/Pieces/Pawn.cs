...
namespace Arya.Chess
{
	public class Pawn : BaseChessPiece
	{
		...
		public Pawn( Chssbrd board, ChessColor color, Point2D position ) : base( board, color, position )
		{
		}
		...
		public override bool CanMoveTo(Point2D newLocation, ref string err)
		{
			...
			else
			{
				...
				else
				{
					...
					BaseChessPiece target = m_Chessboard[ passant ];
		...
		public override ArrayList GetMoves(bool capture)
		{
			...
			if ( m_Chessboard.IsValid( step ) && m_Chessboard[ step ] == null )
			{
				moves.Add( step );
				...
				if ( ! m_HasMoved && m_Chessboard[ step ] == null )
			...
			if ( capture )
			{
				...
				if ( m_Chessboard.IsValid( p1 ) )
				{
					BaseChessPiece piece1 = m_Chessboard[ p1 ];
					...
					else
					{
						...
						if ( m_Chessboard.IsValid( pass1 ) )
						{
							BaseChessPiece passpiece1 = m_Chessboard[ pass1 ];
				...
				if ( m_Chessboard.IsValid( p2 ) )
				{
					BaseChessPiece piece2 = m_Chessboard[ p2 ];
					...
					else
					{
						...
						if ( m_Chessboard.IsValid( p2 ) )
						{
							BaseChessPiece passpiece2 = m_Chessboard[ pass2 ];
