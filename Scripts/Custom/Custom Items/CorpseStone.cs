	...
	public class CorpseStone : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 2 ) ) 
				...
				{
					from.SendGump( new ScriptCorpseGump( m_Price*(from as PlayerMobile).LvL ) );		
		        		from.CantWalk = true;
				}
			}
			else
	...
	{	
		...
		{		   				
			...
			public override void OnResponse( NetState state, RelayInfo info )
			{
				...
				IPooledEnumerable eable = from.Map.GetObjectsInRange( from.Location, 4 );
				...
				foreach( Item item in World.Items.Values)
				{
					if ( item is Corpse )
					{
						if ( (((Corpse)item).Owner == from) && (item.Location != from.Location) )
						{
							corpse = item;
							Found = true;
						}
				...
				//corpse=from.Corpse;
                      	  	if ( (corpse!=null) && !(bodies.Contains(corpse)))
				{
					...
					if ( m_Price > 0 )
					{
			 		...
			 		corpse.Location = new Point3D( from.Location );	
				...
				{
					from.SendMessage( "No corpses found. Your corpse has decayed, or you did not have one to start out with." ); 
					from.CantWalk = false;
				}
			}
		}
