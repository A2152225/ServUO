...
namespace Arya.Auction
{
	...
	public class BidViewGump : Gump
	{
		...
		private void MakeGump()
		{
			...
			AddLabel(12, 5, AuctionLabelHue.kRedHue, AuctionSystem.ST[ 86 ] );

			AddLabel(160, 5, AuctionLabelHue.kGreenHue, string.Format( AuctionSystem.ST[ 18 ] , m_Page + 1, numOfPages ) );
			...
			AddLabel(55, 30, AuctionLabelHue.kGreenHue, AuctionSystem.ST[ 87 ] );
			AddImageTiled(156, 31, 129, 19, 9274);
			AddAlphaRegion(156, 31, 129, 19);
			AddLabel(160, 30, AuctionLabelHue.kGreenHue, AuctionSystem.ST[ 88 ] );
			...
			for ( int i = 0; i < 10; i++ )
			{
				...
				if ( m_Page * 10 + i < m_Bids.Count )
				{
					...
					AddLabel(15, 50 + i * 20, AuctionLabelHue.kLabelHue, ( m_Page * 10 + i + 1 ).ToString() );
					AddLabelCropped( 55, 50 + i * 20, 100, 19, AuctionLabelHue.kLabelHue, bid.Mobile != null ? bid.Mobile.Name : AuctionSystem.ST[ 78 ] );
					AddLabel(160, 50 + i * 20, AuctionLabelHue.kLabelHue, bid.Amount.ToString("#,0" ));
				}
			}

			AddButton(10, 255, 4011, 4012, 0, GumpButtonType.Reply, 0);
			AddLabel(48, 257, AuctionLabelHue.kLabelHue, AuctionSystem.ST[ 89 ] );
