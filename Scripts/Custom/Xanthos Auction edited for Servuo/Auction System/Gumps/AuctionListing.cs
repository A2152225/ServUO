...
namespace Arya.Auction
{
	...
	public class AuctionListing : Gump
	{
		...
		private void MakeGump()
		{
			...
			AddLabel(160, 45, AuctionLabelHue.kGreenHue, AuctionSystem.ST[ 8 ] );
			...
			if ( m_EnableSearch )
			{
				AddLabel(305, 120, AuctionLabelHue.kLabelHue, AuctionSystem.ST[ 16 ] );
				AddButton(270, 120, 4005, 4006, 1, GumpButtonType.Reply, 0);
			}

			// Sort: BUTTON 2
			AddLabel(395, 120, AuctionLabelHue.kLabelHue, AuctionSystem.ST[ 17 ] );
			...
			if ( m_List.Count > 0 )
			{
				...
				AddLabel( 360, 95, AuctionLabelHue.kRedHue, string.Format( AuctionSystem.ST[ 18 ] , m_Page + 1, ( m_List.Count - 1 ) / 10 + 1 ) );
				AddLabel(70, 120, AuctionLabelHue.kRedHue, string.Format( AuctionSystem.ST[ 19 ] , m_List.Count ) );
			}
			else
				AddLabel( 70, 120, AuctionLabelHue.kRedHue, AuctionSystem.ST[ 20 ] );
			...
			if ( m_List.Count > 0 )
			{
				for ( int i = 0; i < 10 && ( m_Page * 10 + i ) < m_List.Count; i++ )
				{
					...
					AddLabelCropped( 140, 150 + i * 20, 260, 20, AuctionLabelHue.kLabelHue, item.ItemName );
				}
			}
			...
			{
				AddLabel(355, 360, AuctionLabelHue.kLabelHue, AuctionSystem.ST[ 22 ] );
			...
			if ( m_Page > 0 )
			{
				AddLabel(180, 360, AuctionLabelHue.kLabelHue, AuctionSystem.ST[ 21 ] );
				AddButton(280, 360, 4014, 4015, 4, GumpButtonType.Reply, 0);
			}
			
			// Close: BUTTON 0
			AddLabel(115, 360, AuctionLabelHue.kLabelHue, AuctionSystem.ST[ 7 ] );
