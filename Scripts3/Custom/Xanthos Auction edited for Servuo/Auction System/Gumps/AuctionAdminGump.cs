...
namespace Arya.Auction
{
	...
	public class AuctionAdminGump : Gump
	{
		...
		private void MakeGump()
		{
			...
			AddLabel(36, 5, Xanthos.Utilities.AuctionLabelHue.kRedHue, @"Auction System Administration");
			AddImageTiled(16, 30, 238, 1, 9274);

			AddLabel(15, 65, AuctionLabelHue.kLabelHue, string.Format( @"Deadline: {0} at {1}", AuctionScheduler.Deadline.ToShortDateString(), AuctionScheduler.Deadline.ToShortTimeString() ) );
			AddLabel(15, 40, AuctionLabelHue.kGreenHue, string.Format( @"{0} Auctions, {1} Pending", AuctionSystem.Auctions.Count, AuctionSystem.Pending.Count ) );

			// B 1 : Validate
			AddButton(15, 100, 4005, 4006, 1, GumpButtonType.Reply, 0);
			AddLabel(55, 100, AuctionLabelHue.kLabelHue, @"Force Verification");

			// B 2 : Profile
			AddButton(15, 130, 4005, 4006, 2, GumpButtonType.Reply, 0);
			AddLabel(55, 130, AuctionLabelHue.kLabelHue, @"Profile the System");

			// B 3 : Temporary Shutdown
			AddButton(15, 160, 4005, 4006, 3, GumpButtonType.Reply, 0);
			AddLabel(55, 160, AuctionLabelHue.kLabelHue, @"Temporarily Shut Down");

			// B 4 : Delete
			AddButton(15, 190, 4005, 4006, 4, GumpButtonType.Reply, 0);
			AddLabel(55, 190, AuctionLabelHue.kLabelHue, @"Permanently Shut Down");

			// B 0 : Close
			AddButton(15, 230, 4023, 4024, 0, GumpButtonType.Reply, 0);
			AddLabel(55, 230, AuctionLabelHue.kLabelHue, @"Exit");
		}
