...
namespace Arya.Auction
{
	...
	public class AuctionMessageGump : Gump
	{
		...
		private void MakeGump()
		{
			...
			if ( auction != null )
			{
				AddLabel(125, 85, AuctionLabelHue.kRedHue, auction.ItemName );

				AddButton(65, 112, 9762, 9763, 0, GumpButtonType.Reply, 0);
				AddLabel(85, 110, AuctionLabelHue.kLabelHue, AuctionSystem.ST[ 27 ] );
			}				
			else
			{
				AddLabel( 125, 85, AuctionLabelHue.kRedHue, AuctionSystem.ST[ 28 ] );
			}
			
			AddHtml( 75, 170, 413, 120, m_HtmlMessage, (bool)true, (bool)false);
			AddLabel(75, 150, AuctionLabelHue.kLabelHue, AuctionSystem.ST[ 29 ] );
			...
			if ( m_InformationMode )
			{
				...
				AddLabel( 125, 355, AuctionLabelHue.kLabelHue, m_OkText );
			}
			else
			{
				AddButton(45, 300, 1147, 1149, 1, GumpButtonType.Reply, 0);
				AddLabel(125, 310, AuctionLabelHue.kLabelHue, m_OkText);
				AddButton(45, 345, 1144, 1146, 2, GumpButtonType.Reply, 0);
				AddLabel(125, 355, AuctionLabelHue.kLabelHue, m_CancelText);
			}

			if ( m_ShowExpiration && auction != null )
			{
				AddLabel( 55, 405, AuctionLabelHue.kRedHue, 
