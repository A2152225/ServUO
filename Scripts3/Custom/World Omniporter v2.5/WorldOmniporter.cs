...
namespace Server.Items
{
	...
	public class WorldOmniporterGump : Gump
	{
		...
		public void PaymentConfirm_Callback( Mobile from, bool okay )
		{
			if ( okay )
			{
				int CosT = m_WO.GetPrice( from, go, pets );
	if (from is PlayerMobile)
	{
				...
				else if ( ((PlayerMobile)(from)).Account.WithdrawGold(CosT))
				{
				...
				Console.WriteLine("Tried to withdraw {0} gold from the account of {1} with a balance of {2}.",CosT,from.Name,((PlayerMobile)(from)).Account.WithdrawCurrency(CosT));
				}			
